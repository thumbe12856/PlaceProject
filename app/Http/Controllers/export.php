...
class export extends BaseController
{
    ...
    function index(Request $request)
    {
        ...
        $section->addText($request->input('name').'企劃書', 'titleStyle', array('align'=>'center'));
        for($i=0; $i<20; $i++) {
            $section->addText(' ', 'titleStyle', array('align' => 'center'));
        }

        $section->addText('活動地點：'.$request->input('location'), 'titleStyle', array('align'=>'center'));
        $section->addText('活動日期：'.$request->input('date'), 'titleStyle', array('align'=>'center'));
        ...
        $section->addText('    '.$request->input('date'), 'contentStyle');

        $section->addText(' ', 'titleStyle', array('align' => 'center'));
        $section->addText('柒、活動地點', 'titleStyle');
        $section->addText('    '.$request->input('location'), 'contentStyle');

        $section->addText(' ', 'titleStyle', array('align' => 'center'));
        $section->addText('捌、活動內容', 'titleStyle');
        $section->addText('    1. 場佈', 'contentStyle');
        $section->addText('    2. 開會', 'contentStyle');
        $section->addText('    3. 場復', 'contentStyle');$section->addText(' ', 'titleStyle', array('align' => 'center'));
        ...
        $section->addText('速別：普通', 'contentStyle');
        $section->addText('密等及解密條件或保密期限：', 'contentStyle');
        $section->addText('附件：', 'contentStyle');

        $section->addText('開會事由：'.$request->input('name'), 'middleTitleStyle' );$section->addText(' ', 'middleTitleStyle', array('align' => 'center'));
        $section->addText('開會時間：'.$request->input('date'), 'middleTitleStyle' );$section->addText(' ', 'middleTitleStyle', array('align' => 'center'));
        $section->addText('開會地點：'.$request->input('location'), 'middleTitleStyle' );$section->addText(' ', 'middleTitleStyle', array('align' => 'center'));
        ...
        $section->addText('出席者：本校各應屆畢業班畢業生代表及畢業紀念冊編輯', 'contentStyle');
        $section->addText('列席者：', 'contentStyle');
        $section->addText('副本：本會行政中心秘書處', 'contentStyle');$section->addText(' ', 'middleTitleStyle', array('align' => 'center'));
        ...
        $section->addText($request->input('name').'會議議程', 'middleTitleStyle', array('align' => 'center'));
        $section->addText('時間：'.$request->input('date'), 'contentStyle');
        $section->addText('地點：'.$request->input('location'), 'contentStyle');
        ...
        $section->addText('壹、 主席報告', array('name'=>'標楷體', 'size'=>12, 'bold'=>true));
        $section->addText('貳、 宣讀上次會議紀錄', array('name'=>'標楷體', 'size'=>12, 'bold'=>true));
        $section->addText('    決　　定：洽悉', 'contentStyle');
        $section->addText('參、報告事項', array('name'=>'標楷體', 'size'=>12, 'bold'=>true, 'bgColor' => 'FFFF00'));
        $section->addText('報告事項一', array('name'=>'標楷體', 'size'=>12, 'bold'=>true, 'bgColor' => 'FFFF00'));
        $section->addText('    報告單位：', array('name'=>'標楷體', 'size'=>12, 'bold'=>false, 'bgColor' => 'FFFF00'));
        $section->addText('    案　　由：', array('name'=>'標楷體', 'size'=>12, 'bold'=>false, 'bgColor' => 'FFFF00'));
        $section->addText('說　　明：', array('name'=>'標楷體', 'size'=>12, 'bold'=>false, 'bgColor' => 'FFFF00'));$section->addText(' ', 'middleTitleStyle', array('align' => 'center'));$section->addText(' ', 'middleTitleStyle', array('align' => 'center'));
        $section->addText('', array('name'=>'標楷體', 'size'=>12, 'bold'=>true));
        $section->addText('肆、 提案討論', array('name'=>'標楷體', 'size'=>12, 'bold'=>true));
        $section->addText('伍、 臨時動議', array('name'=>'標楷體', 'size'=>12, 'bold'=>true));
        $section->addText('陸、 散會', array('name'=>'標楷體', 'size'=>12, 'bold'=>true));$section->addText(' ', 'middleTitleStyle', array('align' => 'center'));$section->addText(' ', 'middleTitleStyle', array('align' => 'center'));
        $section->addText('畢委會信箱：106ccu.graduation@gmail.com', array('name'=>'標楷體', 'size'=>12, 'bold'=>true));
        $section->addText('畢代畢編聯合社團：https://www.facebook.com/groups/1737748126504320/', array('name'=>'標楷體', 'size'=>12, 'bold'=>true));
        $section->addText('畢委會粉絲專頁：https://www.facebook.com/CCUGAC', array('name'=>'標楷體', 'size'=>12, 'bold'=>true));


        $objWriter = \PhpOffice\PhpWord\IOFactory::createWriter($phpWord, 'Word2007');
        $objWriter->save($request->input('name').'場地規劃書.docx');
        return Redirect::to('/'.$request->input('name').'場地規劃書.docx');
    }
}
